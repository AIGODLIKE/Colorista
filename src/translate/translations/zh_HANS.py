...
translations = (
    ...
    ("History Merge Interval", "历史记录合并间隔（秒）", PROP_TCTX),
    (
        "Changes to the same asset made less than this many seconds apart are combined into one history entry; set to 0 to keep each change as a separate entry",
        "同一资产的两次修改若间隔小于该秒数，将合并为一条历史记录；设为 0 则每次修改都单独记录",
        None,
    ),
